...
impl<'ctx> CodeGen<'ctx> {
    pub(super) fn define_list_core(&self) -> Result<(), String> {
        ...
        let ls_cur_a = self
            .builder
            .build_alloca(self.list_type, "cur_a")
            .map_err(llvm_err)?;
        self.builder
        ...
        let ls_cs = self
            .builder
            .build_load(self.list_type, ls_cur_a, "cs")
            .map_err(llvm_err)?
            .into_struct_value();
        let ls_p = self
            .builder
        ...
        let ls_cs2 = self
            .builder
            .build_load(self.list_type, ls_cur_a, "cs2")
            .map_err(llvm_err)?
            .into_struct_value();
        let ls_gv = self
            .builder
        ...
        let ls_result = self
            .builder
            .build_load(self.list_type, ls_cur_a, "result")
            .map_err(llvm_err)?;
        ...
        // ---- action_list_map_walk_rec(ptr node, i64 height, ptr fn, ptr acc) -> void ----
        // In-order B-tree scan: apply callback to each element and push to accumulator.
        ...
        let mwr_cur = self
            .builder
            .build_load(self.list_type, mwr_acc, "mwr_cur")
            .map_err(llvm_err)?
            .into_struct_value();
        let mwr_pushed = self
            .builder
            .build_call(
                push_fn,
                &[mwr_cur.into(), mwr_mapped_bv.into()],
                "mwr_pushed",
            )
            .map_err(llvm_err)?
            .try_as_basic_value()
        ...
        let flatten_fn = self.module.get_function("action_list_flatten").unwrap();
        ...
        let mw_concat = self.context.append_basic_block(mw_fn, "concat");
        ...
        let _ = self
            .builder
            .build_conditional_branch(mw_is_concat, mw_concat, mw_walk);
        self.builder.position_at_end(mw_concat);
        let mw_flat = self
            .builder
            .build_call(flatten_fn, &[mw_list.into()], "mw_flat")
            .map_err(llvm_err)?
            .try_as_basic_value()
            .unwrap_basic()
            .into_struct_value();
        let mw_flat_node = self
            .builder
            .build_extract_value(mw_flat, 0, "mw_flat_node")
            .map_err(llvm_err)?
            .into_pointer_value();
        let mw_flat_h = self
            .builder
            .build_extract_value(mw_flat, 2, "mw_flat_h")
            .map_err(llvm_err)?
            .into_int_value();
        let _ = self
            .builder
            ...
            .map_err(llvm_err)?;
        let mw_flat_res = self
            .builder
            .build_load(self.list_type, mw_acc, "mw_flat_res")
            .map_err(llvm_err)?;
        let _ = self.builder.build_return(Some(&mw_flat_res));
        ...
        // ---- action_list_filter_walk_rec(ptr node, i64 height, ptr fn, ptr acc) -> void ----
        ...
        let fwr_cur = self
            .builder
            .build_load(self.list_type, fwr_acc, "fwr_cur")
            .map_err(llvm_err)?
            .into_struct_value();
        let fwr_pushed = self
            .builder
            .build_call(push_fn, &[fwr_cur.into(), fwr_elem.into()], "fwr_pushed")
            .map_err(llvm_err)?
            .try_as_basic_value()
            .unwrap_basic();
        self.builder
            .build_store(fwr_acc, fwr_pushed)
            .map_err(llvm_err)?;
        ...
        let fw_concat = self.context.append_basic_block(fw_fn, "concat");
        ...
        let _ = self
            .builder
            .build_conditional_branch(fw_is_concat, fw_concat, fw_walk);
        self.builder.position_at_end(fw_concat);
        let fw_flat = self
            .builder
            .build_call(flatten_fn, &[fw_list.into()], "fw_flat")
            .map_err(llvm_err)?
            .try_as_basic_value()
            .unwrap_basic()
            .into_struct_value();
        let fw_flat_node = self
            .builder
            .build_extract_value(fw_flat, 0, "fw_flat_node")
            .map_err(llvm_err)?
            .into_pointer_value();
        let fw_flat_h = self
            .builder
            .build_extract_value(fw_flat, 2, "fw_flat_h")
            .map_err(llvm_err)?
            .into_int_value();
        let _ = self
            .builder
            ...
            .map_err(llvm_err)?;
        let fw_flat_res = self
            .builder
            .build_load(self.list_type, fw_acc, "fw_flat_res")
            .map_err(llvm_err)?;
        let _ = self.builder.build_return(Some(&fw_flat_res));
        ...
        let fd_concat = self.context.append_basic_block(fd_fn, "concat");
        ...
        let _ = self
            .builder
            .build_conditional_branch(fd_is_concat, fd_concat, fd_walk);
        self.builder.position_at_end(fd_concat);
        let fd_flat = self
            .builder
            .build_call(flatten_fn, &[fd_list.into()], "fd_flat")
            .map_err(llvm_err)?
            .try_as_basic_value()
            .unwrap_basic()
            .into_struct_value();
        let fd_flat_node = self
            .builder
            .build_extract_value(fd_flat, 0, "fd_flat_node")
            .map_err(llvm_err)?
            .into_pointer_value();
        let fd_flat_h = self
            .builder
            .build_extract_value(fd_flat, 2, "fd_flat_h")
            .map_err(llvm_err)?
            .into_int_value();
        let _ = self
            .builder
            ...
            .map_err(llvm_err)?;
        let fd_flat_res = self
            .builder
            .build_load(i64, fd_acc, "fd_flat_res")
            .map_err(llvm_err)?;
        let _ = self.builder.build_return(Some(&fd_flat_res));
        ...
        let ay_concat = self.context.append_basic_block(ay_fn, "concat");
        ...
        let _ = self
            .builder
            .build_conditional_branch(ay_is_concat, ay_concat, ay_walk);
        self.builder.position_at_end(ay_concat);
        let ay_flat = self
            .builder
            .build_call(flatten_fn, &[ay_list.into()], "ay_flat")
            .map_err(llvm_err)?
            .try_as_basic_value()
            .unwrap_basic()
            .into_struct_value();
        let ay_flat_node = self
            .builder
            .build_extract_value(ay_flat, 0, "ay_flat_node")
            .map_err(llvm_err)?
            .into_pointer_value();
        let ay_flat_h = self
            .builder
            .build_extract_value(ay_flat, 2, "ay_flat_h")
            .map_err(llvm_err)?
            .into_int_value();
        let ay_flat_hit = self
            .builder
            .build_call(
                ay_rec_fn,
                &[ay_flat_node.into(), ay_flat_h.into(), ay_fn_ptr.into()],
                "ay_flat_hit",
            )
            .map_err(llvm_err)?
            .try_as_basic_value()
            .unwrap_basic()
            .into_int_value();
        let _ = self.builder.build_return(Some(&ay_flat_hit));
        ...
        let al_concat = self.context.append_basic_block(al_fn, "concat");
        ...
        let _ = self
            .builder
            .build_conditional_branch(al_is_concat, al_concat, al_walk);
        self.builder.position_at_end(al_concat);
        let al_flat = self
            .builder
            .build_call(flatten_fn, &[al_list.into()], "al_flat")
            .map_err(llvm_err)?
            .try_as_basic_value()
            .unwrap_basic()
            .into_struct_value();
        let al_flat_node = self
            .builder
            .build_extract_value(al_flat, 0, "al_flat_node")
            .map_err(llvm_err)?
            .into_pointer_value();
        let al_flat_h = self
            .builder
            .build_extract_value(al_flat, 2, "al_flat_h")
            .map_err(llvm_err)?
            .into_int_value();
        let al_flat_ok = self
            .builder
            .build_call(
                al_rec_fn,
                &[al_flat_node.into(), al_flat_h.into(), al_fn_ptr.into()],
                "al_flat_ok",
            )
            .map_err(llvm_err)?
            .try_as_basic_value()
            .unwrap_basic()
            .into_int_value();
        let _ = self.builder.build_return(Some(&al_flat_ok));
