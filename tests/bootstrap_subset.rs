...
/// M181: Point field Bool where Int expected inside PlainBlock rejected.
#[test]
fn test_bootstrap_m181_rejects_bad_plain_block_struct_when_ty() {
    let path = fixtures_root().join("bootstrap_forbidden/bad_plain_block_struct_when_ty.ac");
    let output = run_bootstrap_compiler_on(&path);
    assert!(
        !output.status.success(),
        "bootstrap compiler should exit 1 on bad_plain_block_struct_when_ty.ac (stderr: {})",
        String::from_utf8_lossy(&output.stderr)
    );
}

/// M181: PlainBlock struct+if accepted + Path B allowlist.
#[test]
fn test_bootstrap_m181_allowlisted_plain_block_struct_when_ok() {
    let path = fixtures_root().join("bootstrap/plain_block_struct_when_ok.ac");
    let output = run_bootstrap_compiler_on(&path);
    assert!(
        output.status.success(),
        "bootstrap compiler should accept plain_block_struct_when_ok.ac (stderr: {})",
        String::from_utf8_lossy(&output.stderr)
    );
    assert!(
...
/// M183: unknown enum variant in PlainBlock when rejected.
#[test]
fn test_bootstrap_m183_rejects_bad_plain_block_enum_simple_ty() {
    let path = fixtures_root().join("bootstrap_forbidden/bad_plain_block_enum_simple_ty.ac");
    let output = run_bootstrap_compiler_on(&path);
    assert!(
        !output.status.success(),
        "bootstrap compiler should exit 1 on bad_plain_block_enum_simple_ty.ac (stderr: {})",
        String::from_utf8_lossy(&output.stderr)
    );
}

/// M183: PlainBlock enum+when accepted + Path B allowlist.
#[test]
fn test_bootstrap_m183_allowlisted_plain_block_enum_simple_ok() {
    let path = fixtures_root().join("bootstrap/plain_block_enum_simple_ok.ac");
    let output = run_bootstrap_compiler_on(&path);
    assert!(
        output.status.success(),
        "bootstrap compiler should accept plain_block_enum_simple_ok.ac (stderr: {})",
        String::from_utf8_lossy(&output.stderr)
    );
    assert!(
...
/// M186: return Bool from Int fun via infinite for (call site in PlainBlock) rejected.
#[test]
fn test_bootstrap_m186_rejects_bad_plain_block_infinite_for_return_ty() {
    let path =
        fixtures_root().join("bootstrap_forbidden/bad_plain_block_infinite_for_return_ty.ac");
    let output = run_bootstrap_compiler_on(&path);
    assert!(
        !output.status.success(),
        "bootstrap compiler should exit 1 on bad_plain_block_infinite_for_return_ty.ac (stderr: {})",
        String::from_utf8_lossy(&output.stderr)
    );
}

/// M186: PlainBlock infinite for return accepted + Path B allowlist.
#[test]
fn test_bootstrap_m186_allowlisted_plain_block_infinite_for_return_ok() {
    let path = fixtures_root().join("bootstrap/plain_block_infinite_for_return_ok.ac");
    let output = run_bootstrap_compiler_on(&path);
    assert!(
        output.status.success(),
        "bootstrap compiler should accept plain_block_infinite_for_return_ok.ac (stderr: {})",
        String::from_utf8_lossy(&output.stderr)
    );
    assert!(
        action::driver::BOOTSTRAP_FRONTEND_ALLOWLIST
...
/// M188: no-param lambda body type error inside PlainBlock rejected.
#[test]
fn test_bootstrap_m188_rejects_bad_plain_block_lambda_block_ty() {
    let path = fixtures_root().join("bootstrap_forbidden/bad_plain_block_lambda_block_ty.ac");
    let output = run_bootstrap_compiler_on(&path);
    assert!(
        !output.status.success(),
        "bootstrap compiler should exit 1 on bad_plain_block_lambda_block_ty.ac (stderr: {})",
        String::from_utf8_lossy(&output.stderr)
    );
}

/// M188: PlainBlock no-param brace lambda accepted + Path B allowlist.
#[test]
fn test_bootstrap_m188_allowlisted_plain_block_lambda_block_ok() {
    let path = fixtures_root().join("bootstrap/plain_block_lambda_block_ok.ac");
    let output = run_bootstrap_compiler_on(&path);
    assert!(
        output.status.success(),
        "bootstrap compiler should accept plain_block_lambda_block_ok.ac (stderr: {})",
        String::from_utf8_lossy(&output.stderr)
    );
    assert!(
...
/// M190: lambda body val init type error inside PlainBlock rejected.
#[test]
fn test_bootstrap_m190_rejects_bad_plain_block_lambda_val_ty() {
    let path = fixtures_root().join("bootstrap_forbidden/bad_plain_block_lambda_val_ty.ac");
    let output = run_bootstrap_compiler_on(&path);
    assert!(
        !output.status.success(),
        "bootstrap compiler should exit 1 on bad_plain_block_lambda_val_ty.ac (stderr: {})",
        String::from_utf8_lossy(&output.stderr)
    );
}

/// M190: PlainBlock lambda mid-stmt val accepted + Path B allowlist.
#[test]
fn test_bootstrap_m190_allowlisted_plain_block_lambda_val_ok() {
    let path = fixtures_root().join("bootstrap/plain_block_lambda_val_ok.ac");
    let output = run_bootstrap_compiler_on(&path);
    assert!(
        output.status.success(),
        "bootstrap compiler should accept plain_block_lambda_val_ok.ac (stderr: {})",
        String::from_utf8_lossy(&output.stderr)
    );
    assert!(
...
/// M194: val init type error in PlainBlock keywords shape rejected.
#[test]
fn test_bootstrap_m194_rejects_bad_plain_block_keywords_subset_ty() {
    let path = fixtures_root().join("bootstrap_forbidden/bad_plain_block_keywords_subset_ty.ac");
    let output = run_bootstrap_compiler_on(&path);
    assert!(
        !output.status.success(),
        "bootstrap compiler should exit 1 on bad_plain_block_keywords_subset_ty.ac (stderr: {})",
        String::from_utf8_lossy(&output.stderr)
    );
}

/// M194: PlainBlock keywords subset accepted + Path B allowlist.
#[test]
fn test_bootstrap_m194_allowlisted_plain_block_keywords_subset_ok() {
    let path = fixtures_root().join("bootstrap/plain_block_keywords_subset_ok.ac");
    let output = run_bootstrap_compiler_on(&path);
    assert!(
        output.status.success(),
        "bootstrap compiler should accept plain_block_keywords_subset_ok.ac (stderr: {})",
        String::from_utf8_lossy(&output.stderr)
    );
    assert!(
