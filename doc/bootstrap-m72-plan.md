...
> 当前（M193 后）：**311 passed / 0 failed / 17 ignored**（328 `#[test]`）；allowlist **135** stems。
...
| **M146** | PlainBlock `when` 穷尽 | Path B 对齐 `when_exhaustive` | `plain_block_when_exhaustive_ok` →3；`bad_plain_block_when_exhaustive_ty` → exit 1；allowlist 88 | S | M145 | ✅ |
| **M147** | PlainBlock nullary UFCS | Path B 对齐 `ufcs_len_ok` | `plain_block_ufcs_len_ok` →3；`bad_plain_block_ufcs_ty` → exit 1；allowlist 89 | S | M146 | ✅ |
| **M148** | PlainBlock `or {}` | Path B 对齐 `or_block_ok` | `plain_block_or_ok` →0；`bad_plain_block_or_ty` → exit 1；allowlist 90 | S | M147 | ✅ |
| **M149** | PlainBlock `when` ConditionChain | Path B 对齐 `when_condition_chain` | `plain_block_when_condition_chain_ok` →0；`bad_plain_block_when_chain_ty` → exit 1；allowlist 91 | S | M148 | ✅ |
...
| **M158** | PlainBlock unary `-` | Path B 对齐 `unary_neg_ok` | `plain_block_unary_neg_ok` →2；`bad_plain_block_unary_neg_ty` → exit 1；allowlist 100 | S | M157 | ✅ |
| **M159** | PlainBlock `and`/`or` | Path B 对齐 `logical_ops` | `plain_block_logical_ops_ok` →0；`bad_plain_block_logical_ops_ty` → exit 1；allowlist 101 | S | M158 | ✅ |
| **M160** | PlainBlock assign | Path B 对齐 `assign_expr` | `plain_block_assign_expr_ok` →0；`bad_plain_block_assign_expr_ty` → exit 1；allowlist 102 | S | M159 | ✅ |
...
| **M165** | PlainBlock string `+` | Path B 对齐 `arith_add_string_ok` | `plain_block_arith_add_string_ok` →2；`bad_plain_block_arith_add_string_ty` → exit 1；allowlist 107 | S | M164 | ✅ |
| **M166** | PlainBlock List[String] lit | Path B 对齐 `list_string` | `plain_block_list_string_ok` →3；`bad_plain_block_list_string_ty` → exit 1；allowlist 108 | S | M165 | ✅ |
| **M167** | PlainBlock index key | Path B 对齐 `index_key_ok` | `plain_block_index_key_ok` →0；`bad_plain_block_index_key_ty` → exit 1；allowlist 109 | S | M166 | ✅ |
...
| **M172** | PlainBlock Point let | Path B 对齐 `let_point_ok` | `plain_block_let_point_ok` →0；`bad_plain_block_let_point_ty` → exit 1；allowlist 114 | S | M171 | ✅ |
...
| **M188** | PlainBlock no-param brace lambda | Path B 对齐 `lambda_block_ok`（`once()` 包装 `{ 21 * 2 }()`，调用在 PlainBlock） | `plain_block_lambda_block_ok` →42；`bad_plain_block_lambda_block_ty` → exit 1；allowlist 130 | S | M187 | ✅ |
| **M189** | PlainBlock multi-stmt brace lambda | Path B 对齐 `lambda_stmts_ok`（`once()` 包装 `{ 21; 21 * 2 }()`） | `plain_block_lambda_stmts_ok` →42；`bad_plain_block_lambda_stmts_ty` → exit 1；allowlist 131 | S | M188 | ✅ |
| **M190** | PlainBlock lambda mid-stmt `val` | Path B 对齐 `lambda_val_ok`（`once()` 包装 `{ 21; val a: Int = 21; a * 2 }()`） | `plain_block_lambda_val_ok` →42；`bad_plain_block_lambda_val_ty` → exit 1；allowlist 132 | S | M189 | ✅ |
| **M191** | PlainBlock nested multi-stmt if | Path B 对齐 `if_stmts_ok`（外层 if 包内层 `{ 21; 21 * 2 }`） | `plain_block_if_stmts_ok` →42；`bad_plain_block_if_stmts_ty` → exit 1；allowlist 133 | S | M190 | ✅ |
...
| ID | 目标 | 依赖 |
|----|------|------|
| **M194+** | 下一 Path B / 子集缺口（候选：`keywords_subset` 镜像或 import 深度） | M193 |
...
- [x] PlainBlock `when` 穷尽（M146）
- [x] PlainBlock nullary UFCS（M147）
- [x] PlainBlock `or {}`（M148）
- [x] PlainBlock `when` ConditionChain（M149）
...
- [x] PlainBlock unary `-`（M158）
- [x] PlainBlock `and`/`or`（M159）
- [x] PlainBlock assign（M160）
- [x] PlainBlock arith（M161）
- [x] PlainBlock cmp（M162）
- [x] PlainBlock inclusive for-range（M163）
- [x] PlainBlock coll homo（M164）
- [x] PlainBlock string `+`（M165）
- [x] PlainBlock List[String] lit（M166）
- [x] PlainBlock index key（M167）
- [x] PlainBlock Map lit（M168）
- [x] PlainBlock for + modulo（M169）
- [x] PlainBlock string return/concat（M170）
...
| `doc/bootstrap-subset.md` | 子集允许/禁止 + M4–M170 状态表 |
