...
> 当前（M166 后）：**257 passed / 0 failed / 17 ignored**（274 `#[test]`）；allowlist **108** stems。
...
| ID | 目标 | 依赖 |
|----|------|------|
| **M167+** | 下一 Path B / 子集缺口（候选：`plain_block_index_key_ok` / `plain_block_map_literal_ok`） | M166 |
...
| `doc/bootstrap-subset.md` | 子集允许/禁止 + M4–M166 状态表 |
