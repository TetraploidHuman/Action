...
> Action-in-Action 自举编译器首版**仅允许**使用下列语言特性。随 M4–M195 里程碑扩大（权威状态见 bootstrap-m72-plan）。
...
| M139 | PlainBlock Set for-in | ✅ `plain_block_set_iter_ok`→6；`bad_plain_block_set_iter_ty`；allowlist 82 |
| M140 | PlainBlock `when` | ✅ `plain_block_when_ok`→37；`bad_plain_block_when_ty`；allowlist 83 |
...
| M145 | PlainBlock `print` | ✅ `plain_block_print_ok`→0；`bad_plain_block_print_ty`；allowlist 87 |
| M146 | PlainBlock `when` 穷尽 | ✅ `plain_block_when_exhaustive_ok`→3；`bad_plain_block_when_exhaustive_ty`；allowlist 88 |
| M147 | PlainBlock nullary UFCS | ✅ `plain_block_ufcs_len_ok`→3；`bad_plain_block_ufcs_ty`；allowlist 89 |
| M148 | PlainBlock `or {}` | ✅ `plain_block_or_ok`→0；`bad_plain_block_or_ty`；allowlist 90 |
...
| M153 | PlainBlock String 下标 | ✅ `plain_block_string_index_ok`→0；`bad_plain_block_string_index_ty`；allowlist 95 |
| M154 | PlainBlock `not` | ✅ `plain_block_logical_not_ok`→0；`bad_plain_block_logical_not_ty`；allowlist 96 |
| M155 | PlainBlock `for` exclusive range | ✅ `plain_block_for_range_exclusive_ok`→10；`bad_plain_block_for_range_exclusive_ty`；allowlist 97 |
| M156 | PlainBlock `for` List[String] | ✅ `plain_block_for_string_ok`→6；`bad_plain_block_for_string_ty`；allowlist 98 |
| M157 | PlainBlock unary `+` | ✅ `plain_block_unary_plus_ok`→15；`bad_plain_block_unary_plus_ty`；allowlist 99 |
| M158 | PlainBlock unary `-` | ✅ `plain_block_unary_neg_ok`→2；`bad_plain_block_unary_neg_ty`；allowlist 100 |
| M159 | PlainBlock `and`/`or` | ✅ `plain_block_logical_ops_ok`→0；`bad_plain_block_logical_ops_ty`；allowlist 101 |
| M160 | PlainBlock assign | ✅ `plain_block_assign_expr_ok`→0；`bad_plain_block_assign_expr_ty`；allowlist 102 |
| M161 | PlainBlock arith | ✅ `plain_block_arith_ok`→4；`bad_plain_block_arith_ty`；allowlist 103 |
| M162 | PlainBlock cmp | ✅ `plain_block_cmp_ok`→0；`bad_plain_block_cmp_ty`；allowlist 104 |
| M163 | PlainBlock `for` inclusive range | ✅ `plain_block_for_range_ok`→10；`bad_plain_block_for_range_ty`；allowlist 105 |
| M164 | PlainBlock coll homo | ✅ `plain_block_coll_homo_ok`→0；`bad_plain_block_coll_homo_ty`；allowlist 106 |
| M165 | PlainBlock string `+` | ✅ `plain_block_arith_add_string_ok`→2；`bad_plain_block_arith_add_string_ty`；allowlist 107 |
| M166 | PlainBlock List[String] lit | ✅ `plain_block_list_string_ok`→3；`bad_plain_block_list_string_ty`；allowlist 108 |
| M167 | PlainBlock index key | ✅ `plain_block_index_key_ok`→0；`bad_plain_block_index_key_ty`；allowlist 109 |
| M168 | PlainBlock Map lit | ✅ `plain_block_map_literal_ok`→0；`bad_plain_block_map_literal_ty`；allowlist 110 |
| M169 | PlainBlock for + modulo | ✅ `plain_block_for_modulo_ok`→20；`bad_plain_block_for_modulo_ty`；allowlist 111 |
| M170 | PlainBlock string return/concat | ✅ `plain_block_return_string_concat_ok`→11；`bad_plain_block_return_string_concat_ty`；allowlist 112 |
| M171 | PlainBlock Point assign | ✅ `plain_block_assign_point_ok`→1；`bad_plain_block_assign_point_ty`；allowlist 113 |
| M172 | PlainBlock Point let | ✅ `plain_block_let_point_ok`→0；`bad_plain_block_let_point_ty`；allowlist 114 |
| M173 | PlainBlock Point call | ✅ `plain_block_call_point_ok`→0；`bad_plain_block_call_point_ty`；allowlist 115 |
| M174 | PlainBlock return Point | ✅ `plain_block_return_point_make_ok`→0；`bad_plain_block_return_point_ty`；allowlist 116 |
| M175 | PlainBlock Bool return/cmp | ✅ `plain_block_return_bool_cmp_ok`→1；`bad_plain_block_return_bool_cmp_ty`；allowlist 117 |
| M176 | PlainBlock return Token | ✅ `plain_block_return_token_make_ok`→3；`bad_plain_block_return_token_ty`；allowlist 118 |
| M177 | PlainBlock exclusive range | ✅ `plain_block_range_ok`→3；`bad_plain_block_range_ty`；allowlist 119 |
| M178 | PlainBlock when/if cond | ✅ `plain_block_when_cond_ok`→0；`bad_plain_block_when_cond_ty`；allowlist 120 |
| M179 | PlainBlock custom struct | ✅ `plain_block_custom_struct_ok`→12；`bad_plain_block_custom_struct_ty`；allowlist 121 |
| M180 | PlainBlock many structs | ✅ `plain_block_many_structs_ok`→9；`bad_plain_block_many_structs_ty`；allowlist 122 |
| M181 | PlainBlock struct+if | ✅ `plain_block_struct_when_ok`→0；`bad_plain_block_struct_when_ty`；allowlist 123 |
...
| M186 | PlainBlock infinite for return | ✅ `plain_block_infinite_for_return_ok`→1；`bad_plain_block_infinite_for_return_ty`；allowlist 128 |
| M187 | PlainBlock multi-param lambda | ✅ `plain_block_lambda_multi_ok`→42；`bad_plain_block_lambda_multi_ty`；allowlist 129 |
| M188 | PlainBlock no-param brace lambda | ✅ `plain_block_lambda_block_ok`→42；`bad_plain_block_lambda_block_ty`；allowlist 130 |
| M189 | PlainBlock multi-stmt brace lambda | ✅ `plain_block_lambda_stmts_ok`→42；`bad_plain_block_lambda_stmts_ty`；allowlist 131 |
...
| M194 | PlainBlock keywords subset | ✅ `plain_block_keywords_subset_ok`→0；`bad_plain_block_keywords_subset_ty`；allowlist 136 |
| M195 | PlainBlock jit smoke | ✅ `plain_block_jit_smoke_ok`→42；`bad_plain_block_jit_smoke_ty`；allowlist 137 |

**M72+ 执行计划**：[bootstrap-m72-plan.md](bootstrap-m72-plan.md)（M72–M195 ✅）。
