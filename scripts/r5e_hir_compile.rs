...
impl<'ctx> CodeGen<'ctx> {
    ...
    pub(super) fn compile_hir_stmt(&mut self, stmt: &HirStmt) -> Result<(), String> {
        match stmt {
            ...
            _ => self.compile_hir_stmt_native(stmt)?,
        }
        Ok(())
    }
    ...
    pub(super) fn compile_hir_expr(&mut self, expr: &HirExpr) -> Result<TypedValue<'ctx>, String> {
        match &expr.kind {
            ...
            HirExprKind::Binary(lhs, op, rhs) => self.compile_binary_hir(lhs, *op, rhs),
            HirExprKind::Unary(op, inner) => self.compile_unary_hir(*op, inner),
            HirExprKind::Call {
                func,
                args,
                trailing_lambda,
            } => self.compile_hir_call(func, args, trailing_lambda.as_ref()),
            HirExprKind::When(w) => self.compile_when_hir(w),
            HirExprKind::For(f) => self.compile_for_hir(f),
            HirExprKind::Assign { target, value } => self.compile_assign_hir(target, value),
            HirExprKind::StringInterpolate(parts) => self.compile_string_interp_hir(parts),
            HirExprKind::FieldAccess(obj, field) => self.compile_field_access_hir(obj, field),
            HirExprKind::StructLiteral(fields) => self.compile_struct_lit_hir(fields),
            HirExprKind::MapLiteral(entries) => self.compile_map_lit_hir(entries),
            HirExprKind::SetLiteral(elements) => self.compile_set_lit_hir(elements),
            HirExprKind::Lambda { params, body, .. } => self.compile_lambda_hir(params, body),
    ...
    fn compile_hir_block(&mut self, stmts: &[HirStmt], _result_ty: &Type) -> Result<TypedValue<'ctx>, String> {
        let mut saved = super::Scope::new(); std::mem::swap(&mut self.scope, &mut saved);
        self.scope = super::Scope::with_parent(saved); self.block_did_rc_inc = false;
        let mut last = TypedValue::Unit;
        for s in stmts { match s { HirStmt::Expr { expr, .. } => { self.rc_discard_value(&last)?; last = self.compile_hir_expr(expr)?; } _ => self.compile_hir_stmt(s)? } }
        if self.builder.get_insert_block().ok_or("no block")?.get_terminator().is_none() {
            if self.is_scope_variable(&last) { self.rc_inc_typed_value(&last)?; self.block_did_rc_inc = true; } else { self.block_did_rc_inc = false; }
            self.emit_scope_cleanup()?; } else { self.block_did_rc_inc = false; }
        let mut parent = super::Scope::new(); std::mem::swap(&mut self.scope, &mut parent);
        if let Some(p) = parent.parent { self.scope = *p; } Ok(last) }

    fn compile_hir_call(&mut self, func: &HirExpr, args: &[HirExpr], trailing_lambda: Option<&Box<HirExpr>>) -> Result<TypedValue<'ctx>, String> {
        self.compile_call_hir(func, args, &trailing_lambda.cloned()) }
    ...
    fn compile_binary_values(
        ...
        match op {
            ...
            BinaryOp::Range | BinaryOp::RangeExclusive => {
                ...
                let start_int = match &left.kind {
                    TypedValue::Int(v) => *v,
                    _ => return Err("Range start must be integer".into()),
                };
                let end_int = match &right.kind {
                    TypedValue::Int(v) => *v,
