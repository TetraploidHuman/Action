// Fused map+fold: single B-tree walk (map each element, fold into acc).

use crate::{llvm_err, CodeGen};
use inkwell::IntPredicate;

impl<'ctx> CodeGen<'ctx> {
    pub(in crate::runtime_decl) fn define_list_map_fold(&self) -> Result<(), String> {
        let i64 = self.i64_ty();
        ...
        let map_fn = rec_fn.get_nth_param(2).unwrap().into_pointer_value();
        ...
        let mapped_bv = mapped_call
            .try_as_basic_value()
            .basic()
            .ok_or("map_fold_walk map failed")?;
        ...
        let new_acc = self
            ...
            .try_as_basic_value()
            .basic()
            .ok_or("map_fold_walk fold failed")?
            .into_int_value();
        ...
        let o_map = outer_fn.get_nth_param(1).unwrap().into_pointer_value();
