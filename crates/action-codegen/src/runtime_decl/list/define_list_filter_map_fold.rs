// Fused filter+map+fold: single B-tree walk (filter, map survivors, fold into acc).

use crate::{llvm_err, CodeGen};
use inkwell::IntPredicate;

impl<'ctx> CodeGen<'ctx> {
    pub(in crate::runtime_decl) fn define_list_filter_map_fold(&self) -> Result<(), String> {
        let i64 = self.i64_ty();
        ...
        let leaf_map = self.context.append_basic_block(rec_fn, "leaf_map");
        ...
        let map_fn = rec_fn.get_nth_param(3).unwrap().into_pointer_value();
        let fold_fn = rec_fn.get_nth_param(4).unwrap().into_pointer_value();
        let acc_p = rec_fn.get_nth_param(5).unwrap().into_pointer_value();
        ...
        let pred_bv = pred_call
            .try_as_basic_value()
            .basic()
            .ok_or("filter_map_fold_walk filter failed")?;
        ...
        let _ = self
            .builder
            .build_conditional_branch(is_true, leaf_map, leaf_next);

        self.builder.position_at_end(leaf_map);
        let mapped_call = self
            .builder
            .build_indirect_call(lambda_fn_ty, map_fn, &[elem_tag.into()], "mapped")
            .map_err(llvm_err)?;
        let mapped_bv = mapped_call
            .try_as_basic_value()
            .basic()
            .ok_or("filter_map_fold_walk map failed")?;
        let mapped_tag = if mapped_bv.is_struct_value() {
            self.builder
                .build_extract_value(mapped_bv.into_struct_value(), 0, "mtag")
                .map_err(llvm_err)?
                .into_int_value()
        } else {
            mapped_bv.into_int_value()
        };
        let cur_acc = self
            .builder
        ...
        let new_acc = self
            ...
            .build_indirect_call(
                fold_fn_ty,
                fold_fn,
                &[cur_acc.into(), mapped_tag.into()],
                "folded",
            )
            .map_err(llvm_err)?
            .try_as_basic_value()
            .basic()
            .ok_or("filter_map_fold_walk fold failed")?
            .into_int_value();
        ...
        let o_map = outer_fn.get_nth_param(2).unwrap().into_pointer_value();
        let o_fold = outer_fn.get_nth_param(3).unwrap().into_pointer_value();
        let o_init = outer_fn.get_nth_param(4).unwrap().into_int_value();
        let o_node = self
            .builder
