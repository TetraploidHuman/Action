// Submodule: runtime_decl/define_lazy_list
//
// LazyList → List materialization (moved from codegen inline IR).
...
impl<'ctx> CodeGen<'ctx> {
    pub(super) fn define_lazy_list(&self) -> Result<(), String> {
        ...
        let ll_fn = self.module.add_function(
            "action_lazylist_to_list",
            self.list_type.fn_type(&[self.lazylist_type.into()], false),
            None,
        );
        let entry = self.context.append_basic_block(ll_fn, "entry");
        self.builder.position_at_end(entry);

        let ll_struct = ll_fn.get_first_param().unwrap().into_struct_value();
        let head_val = self
            .builder
        ...
        let make_int_fat = |builder: &inkwell::builder::Builder<'ctx>,
         ...
         -> Result<inkwell::values::StructValue<'ctx>, String> {
            let undef = string_type.get_undef();
            let null_ptr = ptr_ty.const_null();
            let aggregate = builder
                .build_insert_value(undef, val, 0, "fat_v")
                .map_err(llvm_err)?;
            let aggregate2 = builder
                .build_insert_value(aggregate, null_ptr, 1, "fat_p")
                .map_err(llvm_err)?;
            Ok(aggregate2.into_struct_value())
        };

        let load_list = |builder: &inkwell::builder::Builder<'ctx>,
                         list_type: inkwell::types::StructType<'ctx>,
                         ptr: inkwell::values::PointerValue<'ctx>|
         -> Result<inkwell::values::StructValue<'ctx>, String> {
            builder
                .build_load(list_type, ptr, "list_ld")
                .map_err(llvm_err)
        ...
        let final_list = load_list(&self.builder, self.list_type, result_alloca)?;
        let _ = self
            .builder
            .build_return(Some(&final_list as &dyn BasicValue));
        Ok(())
    }
}
