...
impl<'ctx> CodeGen<'ctx> {
    ...
    /// HIR-native generic call: compile args once via `compile_hir_expr`.
    pub(super) fn compile_generic_call_hir(
        &mut self,
        stmt: &Stmt,
        name: &str,
        args: &[HirExpr],
        ...
        for (i, param) in params.iter().enumerate() {
            ...
            if matches!(
                args[i].kind,
                action_frontend::hir::HirExprKind::Lambda { .. }
            ) {
                continue;
            }
            let param_ty = param.ty.clone().unwrap_or(Type::Named("Int".into()));
        ...
        let mut ca: Vec<BasicMetadataValueEnum> = Vec::new();
        for (i, a) in args.iter().enumerate() {
            let bv = self.compile_and_load_hir(a)?;
            let casted = self.coerce_arg(bv, llvm_param_tys.get(i))?;
            ca.push(casted.into());
        }
        if let Some(lam) = trailing {
