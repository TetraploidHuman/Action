...
- **M72+**：见 `doc/bootstrap-m72-plan.md`（M72–M137 ✅；含 nullary UFCS、`or {}`、lambda/`it`/多参/trailing/无参块/多语句/体内 val、if/or/`val`/`return`/`for-*`/`break`/`continue` PlainBlock、Map for-in 键/值绑定、嵌套 for、开放 import 图 + fixtures 搜索根、funSig）
