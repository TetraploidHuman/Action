...
`bootstrap_subset`：**174+ passed / 17 ignored**（含 M4–M20、TC1–TC10、M72–M128、AOT/JIT 子进程隔离；allowlist 70 stems）。
...
- **M72+**：见 `doc/bootstrap-m72-plan.md`（M72–M128 ✅；含 nullary UFCS、`or {}`、lambda/`it`/多参/trailing/无参块/多语句/体内 val、if/or/`val` PlainBlock、开放 import 图 + fixtures 搜索根、funSig）
