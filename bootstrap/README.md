...
见 `doc/bootstrap-subset.md`。**M27–M71** ✅；**M72–M174** ✅（权威状态见 [`doc/bootstrap-m72-plan.md`](../doc/bootstrap-m72-plan.md)）。
...
`bootstrap_subset`：**273 passed / 17 ignored**（含 M4–M20、TC1–TC10、M72–M174、AOT/JIT 子进程隔离；allowlist 116 stems）。
...
- **M72+**：见 `doc/bootstrap-m72-plan.md`（M72–M174 ✅；含 nullary UFCS、`or {}`、lambda/`it`/多参/trailing/无参块/多语句/体内 val、if/or/`val`/`return`/`for-*`/`..`/`..<`/List[String]/`break`/`continue` PlainBlock、Map for-in 键/值/`k,v`/下标读、String 下标、`not`/`and`/`or`、unary `+`/`-`、arith、string `+`、cmp、assign、Point let/assign/call/return、Set for-in、`when`/ConditionChain(+and)/guard/穷尽、字段/下标赋值、`print`、嵌套 for、开放 import 图 + fixtures 搜索根、funSig）
